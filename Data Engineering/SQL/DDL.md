...
3. PROMARY KEY로 지정된 E_ID에 NULL값을 넣어서 기본키의 최소성을 확인하고 중복된 값도 넣어서 기본키의 유일성도 확인
4. HIRE_DATE에 NULL을 넣어서 DEFAULT로 지정한 값이 잘 입력되는지 확인

```SQL
INSERT INTO jhj_emp(EID, PHONE, HIRE_DATE, ENO, ENAME, MARRIAGE, AGE)
VALUES(0, '010-0000-4141', '2022-09-29', '000000-1111111', 'JHJ', 'Z', 19);

INSERT INTO jhj_emp(EID, PHONE, HIRE_DATE, ENO, ENAME, MARRIAGE, AGE)
VALUES(0, '010-0000-4141', '2022-09-29', '00000-1111111', 'JHJ', 'N', 21);

INSERT INTO jhj_emp(PHONE, HIRE_DATE, ENO, ENAME, MARRIAGE, AGE)
VALUES('010-0000-4141', '2022-09-29', '000000-1111111', 'JHJ', 'N', 21);

INSERT INTO jhj_emp(EID, ENO, ENAME, MARRIAGE, AGE)
VALUES(1, '000000-1111112', 'JHJ', 'N', 21);
COMMIT;

-- 테이블 생성 시 데이터 복제
CREATE TABLE CP_JHJ_EMP
AS SELECT * FROM EMPLOYEE WHERE ENT_YN = 'Y';

DROP TABLE CP_JHJ_EMP;

-- 테이블 컬럼만 복제해오기
CREATE TABLE CP_JHJ_EMP
AS SELECT * FROM EMPLOYEE WHERE 1=0;
SELECT * FROM CP_JHJ_EMP;
```
<hr>
...
```SQL
ALTER TABLE JHJ_EMP ADD(JOB_CODE CHAR(2));
```

### 컬럼 수정 MODIFY
- 컬럼의 타입은 테이블에 컬럼 값이 하나도 없는 경우에만 변경이 가능
- 컬럼의 크기는 지금보다 큰 크기로만 변경이 가능
- 제약조건을 지정할 경우 해당 제약조건을 어긴 데이터가 존재해서는 안된다.
    - NOT NULL, 단 NULL인 데이터가 단 하나도 없을 때만 지정 가능
    - UNIQUE, 단 중복 데이터가 없을 때만 가능

```SQL
ALTER TABLE JHJ_EMP MODIFY(JOB_CODE NOT NULL);
ALTER TABLE JHJ_EMP MODIFY(JOB_CODE CHAR(10));
ALTER TABLE JHJ_EMP MODIFY(JOB_CODE UNIQUE);
```

### 컬럼 삭제 DROP

```SQL
ALTER TABLE JHJ_EMP DROP COLUMN JOB_CODE;
```

### 테이블 삭제 DROP

- 자식 테이블이 있는 경우 테이블 삭제 불가능
- 참조 제약조건을 삭제하면 테이블을 삭제 하고 싶을 경우

```SQL
DROP TABLE JHJ_EMP CASCADE CONSTRAINTS;
DROP TABLE JHJ_EMP;
```